...
describe('auto add label', () => {
  ...
  test('getType', () => {
    ...
    expect(getType('fix(hack): fix a bug in a hacking way')).toBe(defaultConfig.fix.hack)
    expect(getType('fix($core): fix a serious bug')).toBe(defaultConfig.fix.default)

    expect(getType('chore: do homework')).toBe(defaultConfig.chore.default)
    expect(getType('chore(deps): upgrade library')).toBe(defaultConfig.chore.deps)
    ...
    expect(getType('perf: reduce network requests')).toBe(defaultConfig.perf.default)
